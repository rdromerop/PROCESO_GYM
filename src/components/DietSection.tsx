...
export const DietSection = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
      <GlassCard title="Plan de Alimentación" className="lg:col-span-2 card-orange">
        <div className="flex flex-col gap-4 mt-2">
          {meals.map((meal, idx) => (
            <div key={idx} className="flex gap-4 p-4 rounded-xl hover:bg-zinc-900/50 transition-all border border-transparent hover:border-zinc-800">
              <div className="text-sm font-bold text-primary w-12 pt-1">{meal.time}</div>
              <div className="flex flex-col gap-1">
                <span className="font-bold text-white tracking-tight">{meal.name}</span>
      ...
      <GlassCard title="Snacks Permitidos" className="card-blue">
        <div className="flex flex-col gap-3 mt-2">
          {snacks.map((snack, idx) => (
            <div key={idx} className="flex items-center justify-between p-4 rounded-xl border border-zinc-800 bg-zinc-900/30 hover:border-secondary/30 transition-all">
              <div className="flex items-center gap-3">
                <div className="text-secondary">{snack.icon}</div>
